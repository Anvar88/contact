...
@section('content')
    ...
    <table class="table table-bordered">
        ...
        <form action="{{ route('search.store') }}" method="POST">
            ...
            <th><input type="text" name="name" value="{{$request->name }}" class="form-control" placeholder="Имя контакта"></th>

            <th><input type="text" name="phone" value="{{$request->phone  }}" class="form-control" placeholder="Номер телефона"></th>
            <th>
                    <select name="id_group" id="id_group" title="Группа" >
                        <option   value="" >Выберите группу </option>
                        @foreach($groups as $key=>$value)
                            <option {{ $request->id_group == $key ? "selected=selected" : "" }} value="{{$key}}">{{$value}}</option>
                        @endforeach
                    </select>
            </th>
             @csrf
